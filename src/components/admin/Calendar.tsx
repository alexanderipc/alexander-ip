...
import { getCalendarData, type CalendarEvent } from "@/app/admin/actions";
...
export default function Calendar() {
  ...
  function fetchMonth(y: number, m: number) {
    startTransition(async () => {
      try {
        const data = await getCalendarData(y, m + 1); // server expects 1-indexed month
        setEvents(data);
      } catch (err) {
