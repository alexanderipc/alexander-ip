...
export default function AdminMessageThread({
  ...
  function handleSend() {
    ...
    startTransition(async () => {
      try {
        const res = await sendAdminMessage(projectId, trimmed, sentAttachments);
        if (res?.message) {
              ...
              >
                <div
                  className={`max-w-[80%] rounded-xl px-4 py-2.5 ${
                    isAdmin ? "bg-navy text-white" : "bg-slate-100 text-slate-800"
